...
<p class="h1"><b>Welcome <? echo $username;?></b> </p>

<table width="100%" cellspacing=0>
	<tr>
		<td align="left" class="h2"><span class="style1">Submit Your Payment Detail</span></td>
...
//sendMail_paper_upload($username,trim($_POST["project"]),trim($_POST["institution"],$time,$_FILES["abstract_file"]["name"],$_FILES["full_paper"]["name"],$status,$payment,$pwd)

/*    for add co-author
while (list($key,$value) = each($accountName)) { 
	$text .= $value.$br;	
}
 */      
 /*
$sql = " INSERT INTO tiim_paper(register_name,project_name,description,institution,abstract,full_paper,date,status,payment) VALUES (";
                $sql .= " '".$username."'";
                $sql .= ",'".trim($_POST["project"])."'";
                $sql .= ",'".trim($_POST["description"])."'";
                $sql .= ",'".trim($_POST["institution"])."'";
				if (is_uploaded_file($_FILES['abstract_file']['tmp_name'])) 			
                		$sql .= ",'".$_FILES["abstract_file"]["name"].'.'.mktime($time)."'";
				else 	$sql .= ",'' ";
				if (is_uploaded_file($_FILES['full_paper']['tmp_name'])) 
		                $sql .= ",'".$_FILES["full_paper"]["name"].'.'.mktime($time)."'";
				else 	$sql .= ",'' ";				
                $sql .= ",'".$time."'";
                $sql .= ",'New'";		
                $sql .= ",'Not paid'";							
...
if( ! is_dir('uploads/'.$username.'/abstract'))
	mkdir('uploads/'.$username.'/abstract');
if( ! is_dir('uploads/'.$username.'/fullpaper'))
	mkdir('uploads/'.$username.'/fullpaper');	

$uploads_dir = 'uploads/'.$username.'/abstract';
...
$uploads_dir = 'uploads/'.$username.'/fullpaper';
if (is_uploaded_file($_FILES['full_paper']['tmp_name'])) {
        $tmp_name = $_FILES["full_paper"]["tmp_name"];
        $name = $_FILES["full_paper"]["name"].'.'.mktime($time);
        move_uploaded_file($tmp_name, "$uploads_dir/$name") or die("ERROR: Can not upload full paper file.");
		sendMail_paper_upload($to,$project,$institution,$date,$abstract,$full_paper,$status,$payment,$fname,$lname )
		echo '<p>Full paper file upload success.</p>';
}


echo '<p><br><a herf="main.php" onclick="window.history.go(-1)"><input type="submit" value="Go Back" /></a></p>';
*/
if($pa){ 	//update paper
//echo $count; } 
//echo $id;
	
	 	$sql = "UPDATE tiim_paper SET description='$des',institution='$in' WHERE id ='$id' ";
		$result = @mysql_query($sql) or die(mysql_error());
		}
if($co){		//insert co-author
	$ref_id = $id;
$count = count($title1); 
  for($i=0;$i<$count;$i++){
...
//echo '<p><br><a herf="main.php?type=edit&papercode=$papercode" onclick="window.history.go(-1)"><input type="submit" value="Go Back" /></a></p>';
}

if($paper){
	$sql = "SELECT * FROM tiim_paper_runno;";

		$result = @mysql_query($sql) or die(mysql_error());
		
		$prefix = @mysql_result($result,0,"pre_code");
		$runno = @mysql_result($result,0,"runno");				
		mysql_free_result($result);
		$project = $prefix.$runno;
		
		$next = $runno+1;
		
		$sql = "UPDATE tiim_paper_runno SET runno=' ".$next." ' WHERE id =1;";

		$result = @mysql_query($sql) or die(mysql_error());


$sql = " INSERT INTO tiim_paper(register_name,project_name,description,institution,date,payment) VALUES (";
                $sql .= " '".$username."'";
                $sql .= ",'".trim($project)."'";
                $sql .= ",'".trim($_POST["description"])."'";
                $sql .= ",'".trim($_POST["institution"])."'";			
...
$count = count($title); 
 for($i=0;$i<$count;$i++){
	$sql = " INSERT INTO tiim_co_author (ref_id,title,firstname,lastname,telephone,email) VALUES (";
					$sql .= " '".$ref_id."'";
					$sql .= ",'".$title[$i]."'";
					$sql .= ",'".trim($FirstName[$i])."'";
					$sql .= ",'".trim($LastName[$i])."'";			
					$sql .= ",'".trim($Telephone[$i])."'";
					$sql .= ",'".trim($email[$i])."'";							
					$sql .= ")";				
				$result = mysql_query($sql) or die(mysql_error());		
				//sendMail_paper_upload($to,$project,$institution,$date,$abstract,$full_paper,$status,$payment,$fname,$lname )
				sendMail_co_author(trim($email[$i]),trim($FirstName[$i]),trim($LastName[$i]));								
}				

print( "<script language=javascript> alert(\"Your paper code : ".$project."\"); </script>");
...
if($abstract){
	//echo $paper_id;
	$sql = " INSERT INTO tiim_abstract_paper(ref_id,abstract,date,status) VALUES (";
                $sql .= " '".$paper_id."'";
                if (is_uploaded_file($_FILES['abstract_file']['tmp_name'])) 			
                		$sql .= ",'".$_FILES["abstract_file"]["name"].'.'.mktime($time)."'";
				else 	$sql .= ",'' ";
	...
	/*
	if( ! is_dir('uploads/'.$username.'/'.$paper_id))
		mkdir('uploads/'.$username.'/'.$paper_id);
		
	if( ! is_dir('uploads/'.$username.'/'.$paper_id.'/abstract'))
		mkdir('uploads/'.$username.'/'.$paper_id.'/abstract');
	*/		
...
if($fullpaper){
	//echo $paper_id;
	$sql = " INSERT INTO tiim_full_paper(ref_id,full_paper,date,status) VALUES (";
                $sql .= " '".$paper_id."'";
               	if (is_uploaded_file($_FILES['full_paper']['tmp_name'])) 
		                $sql .= ",'".$_FILES["full_paper"]["name"].'.'.mktime($time)."'";
				else 	$sql .= ",'' ";			
	...
	if( ! is_dir('uploads/'.$username.'/'.$paper_id))
		mkdir('uploads/'.$username.'/'.$paper_id);
	if( ! is_dir('uploads/'.$username.'/'.$paper_id.'/fullpaper'))
		mkdir('uploads/'.$username.'/'.$paper_id.'/fullpaper');
	*/
	$path = 'uploads/'.$username.'/';
	//echo $path;
	if(!(@opendir($path)))
	...
	$path = 'uploads/'.$username.'/'.$paper_id.'/fullpaper/';
	//echo $path;
	if(!(@opendir($path)))
		mkdir ($path, 0777);	
			
	$uploads_dir = 'uploads/'.$username.'/'.$paper_id.'/fullpaper';
...
if($payment){
	...
	$sql = " INSERT INTO tiim_payment(register_name, method, bank_date, bank_owner, bank_transfer, bank_amount, bank_slip, date, status) VALUES (";
                $sql .= " '".$my_username."'";
				$sql .= ",'1'";
				$sql .= ",'".$bank_date."'";
				//$sql .= ",'".$bank_time."'";
				...
				$sql .= ",'".$bank_transfer."'";
				$sql .= ",'".$bank_amount."'";
                if (is_uploaded_file($_FILES['bank_slip']['tmp_name']))
                		$sql .= ",'".$_FILES["bank_slip"]["name"]."'";
				else 	$sql .= ",'' ";
                $sql .= ",'".$time."'";
                $sql .= ",'Informed Payment'";							
                $sql .= ")";
	//echo $sql;			
	$result = mysql_query($sql) or die(mysql_error());		
	
	$sql = "UPDATE tiim_paper SET payment='Informed Payment' WHERE register_name ='$my_username' ";
	$result = @mysql_query($sql) or die(mysql_error());
	...
	if( ! is_dir('uploads/'.$username.'/'.$paper_id.'/abstract'))
		mkdir('uploads/'.$username.'/'.$paper_id.'/abstract');
	*/		
	$uploads_dir = 'uploads/'.$username.'/payslip';
	
	//echo $uploads_dir;
	
	if (is_uploaded_file($_FILES['bank_slip']['tmp_name'])) {
			$tmp_name = $_FILES["bank_slip"]["tmp_name"];
			//$name = $_FILES["bank_slip"]["name"].'.'.mktime($time);
			$name = $_FILES["bank_slip"]["name"];
			move_uploaded_file($tmp_name, "$uploads_dir/$name") or die("ERROR: Can not upload Bank slip.");
			//sendMail_paper_upload($to,$project,$institution,$date,$abstract,$full_paper,$status,$payment,$fname,$lname )
			echo '<p>Bank slip file upload success.</p>';
	}
	
}

echo '<p><br><a href="../payment.html" >Go Back</a></p>';
