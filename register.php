...
{ 
		...
		if($numrow>0)
		{    
		echo"<script>alert('This Email $email Already exists');  onclick=window.history.go(-1)</script>"; 
    exit;
		}	
}
?>
...
/*if (trim($_POST["user_code"])=='') {
		echo '<strong>Missing:</strong>Code Number<br>';
		$error=1;
}
if (trim($_POST["user_code"]) != '') {
		echo '<strong>Missing:</strong>Not Like Code Number <br>';
		$error=1;
}*/
//---------------------
...
echo '<strong>Missing:</strong> Invalid code number';
 $error=1;
}
//---------------------
if ($error==1) {
	die('<br><a herf="registration.html" onclick="window.history.go(-1)"><input type="submit" value="Go Back" /></a>');
}

?><?
//------------------------------------------- runno user id
$maer = mysql_query("SELECT * FROM tiim_user_runno ");
//$result = mysql_query("SELECT * FROM Customers ORDER BY Age");
$row = mysql_fetch_array($maer);
 $user_run = $row['runno']+1;

 mysql_query("UPDATE tiim_user_runno SET runno ='".$user_run."'");

//----------------------------------
$user_id=date(ymd).$user_run;
$sql=mysql_query("SELECT * FROM tiim_user  ORDER BY  user_id");
$pwd =  createPassword(trim($_POST["email"]));
$reg_text = " ";
if($_POST["reg_type"]=="2"){ 
$reg_text= $_POST["reg_text1"]; 
}
else if($_POST["reg_type"]=="3") {
$reg_text= $_POST["reg_text2"];
 }

$sql = "INSERT INTO tiim_user (status,first_name,last_name,title,institution,address,city,postal_code,state_province,country,phone,fax,email,pwd,reg_type,reg_text,pay,user_id ) VALUES (";
...
if ($result == 1){
	...
	//sendMail_registered(trim($_POST["email"]),$pwd,trim($_POST["firstname"]),trim($_POST["lastname"]),trim($url));
