...
<body> 
  ...
  <table width="82%" border="0" align="center" cellpadding="1" cellspacing="0" class="admin_table">
    ...
    <tr>
      ...
      <td valign="top" ><input name="institution_co" type="text" value="<?=$row['institution_co'] ?>" /></td>
    </tr> 
	 <tr>
       <td valign="top" class="field_label" bgcolor="#EFEFEF">Country:</td>
       <td valign="top" bgcolor="#EFEFEF" ><label><input name="country_co" type="text" value="<?=$row['country_co'] ?>" /> </label></td>
    </tr> 
    ...
    <tr>
      ...
      <td width="252"><label>
	   ...
	   <?
	 	  ...
	 	  if($Submit<>"")
	  	  { 	  
	  	$sql1 = "UPDATE tiim_co_author SET 	  title='$title',firstname='$firstname',lastname='$lastname',email='$email' ,institution_co='$institution_co',country_co='$country_co' where id='$id' ";

		$result1 = @mysql_query($sql1) or die(mysql_error(ไม่ได้));
		echo "<script>window.close()</script>"; ?>
	    <script>opener.location.reload()</script>  
		sendMail_co_author(trim($email),trim($FirstName),trim($LastName));		
		<!-- echo "<script>window.close()</script>"; ?>
	    <script>opener.location.reload()</script>  -->
	 <? } ?>
	  </td>
