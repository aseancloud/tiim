...
<table width="100%" cellspacing=0>
	<tr>
		<td align="left" class="h2"><span class="style1">Upload Your New Papers</span></td>
...
$count = count($title);
for($i=0;$i<$count;$i++){
	$sql = " INSERT INTO tiim_co_author (ref_id,title,firstname,lastname,institution_co,country_co,telephone,email) VALUES (";
					$sql .= " '".$ref_id."'";
					$sql .= ",'".$title[$i]."'";
					$sql .= ",'".trim($FirstName[$i])."'";
					$sql .= ",'".trim($LastName[$i])."'";	
					$sql .= ",'".trim($Institution_co[$i])."'";	
					$sql .= ",'".trim($Country_co[$i])."'";			
...
?>                 
   ...
   <p><br><input type="button" name="back" value="Go Back" onclick="location.href=('main.php')"  /></p>             
